...
export default function ManualWorkoutLogScreen() {
  ...
  const loadEntries = useCallback(async () => {
    ...
    try {
      const cachedEntries = await AsyncStorage.getItem(`@fittrax_workout_entries_${userId}`);
      const cachedTemplates = await AsyncStorage.getItem(`@fittrax_workout_templates_${userId}`);
      ...
      const [entriesRes, templatesRes] = await Promise.all([
        axios.get(`${API_URL}/api/manual-workout-log/${userId}`, { timeout: 5000 }).catch(() => null),
        axios.get(`${API_URL}/api/workout-templates/${userId}`, { timeout: 5000 }).catch(() => null),
      ]);
      
      if (entriesRes?.data?.entries) {
        setEntries(entriesRes.data.entries);
        await AsyncStorage.setItem(`@fittrax_workout_entries_${userId}`, JSON.stringify(entriesRes.data.entries));
      }
      if (templatesRes?.data?.templates) {
        setTemplates(templatesRes.data.templates);
        await AsyncStorage.setItem(`@fittrax_workout_templates_${userId}`, JSON.stringify(templatesRes.data.templates));
      }
    } catch (error) {
      console.log('Offline mode: Using cached workout data');
    } finally {
      setLoading(false);
    }
  }, [userId]);
